...
### Fixed
- A tooltip no longer stays on screen after the element it describes is gone. The popup is dismissed when the pointer leaves that element, so an element removed or rebuilt by a refresh dismissed nothing at all - on the filter chips, which are rebuilt on every refresh, a tooltip could stay standing indefinitely. Tooltips now follow their element: rebuilt in place, the tooltip stays put; genuinely gone, so is the tooltip.
...
### Changed
- The row menu's scratchpad entry and the tooltip on a project's path now read *Show scratchpad in Explorer* and *Show in Explorer*. Both raise an Explorer window, exactly like the new transcript entry, so all three now say that in the same words instead of mixing "open" and "show" for the same kind of action.
- A session stopped by an API error now says what stopped it. The status read a bare "Error" for everything except a usage limit, so the one thing you needed to know - whether to wait, retry, or sign in again - was only in the session's own window: an overloaded server, a rejected request, and an expired login all looked identical. The status and its tooltip now name the cause as far as the error record says ("Error: servers overloaded", "Error: authentication failed", or "Error: HTTP 521" when only the status code is known), and stay a plain "Error" when it says nothing usable. A usage limit is still called out as before. Hovering the status dot adds the error's own message line beneath that label, which is where the details only the message carries show up - when a usage limit resets, or which request was rejected; it is Claude Code's own wording, in English, and one line long.
- An empty result now says which of the two reasons emptied it. A search that matched nothing reported "No agents match this filter", which blamed the filter chips for a search result and sent people to the wrong control; it now reads "The search text was not found in any session", with a line beneath it naming whatever limited the scan - the sessions your chips hide, or the window the *Older* chip is set to.

### Added
- A session's row menu can now show its transcript file in Windows Explorer, selected in its folder. Useful whenever you want the raw `.jsonl` itself - to archive it, diff it, or hand it to another tool - without hunting through `~/.claude/projects` for the right project folder. The file is only ever shown, never opened, so nothing is launched for it; the entry appears only for a session that has already written a transcript.
- A search that finds nothing now offers to look wider instead of leaving you guessing. Because the search only ever reads the sessions your filter chips show, the one you are looking for can be sitting just outside them - a finished session under a chip you switched off, or one older than the *Older* window. The empty result now offers a single button that widens the search by one step: first to the sessions the chips hide, then one time window further back. Each click costs only that step, and the sessions it finds are shown even though their chip is off, with a line above the list saying how many. Your chips are never changed - the extra rows belong to that one query and disappear as soon as you edit the search text, or right away via the *Reset* on that same line if you want your filters back without retyping. Anything found this way keeps updating live, exactly like the rest of the list.
...
### Changed
- The *History* chip is now called *Older*. "History" is the browser's word for everything you have ever opened, including what is open right now; this chip is the opposite of that - only the sessions that have dropped out of the overview and appear under none of the other chips. Its colour, its place in the row, and everything it does are unchanged.
- The *Older* chip now reaches back a chosen distance instead of all the way: its right half opens a window picker (last hour, 24 hours, 7 days, 30 days, or everything) and starts at 24 hours. On a machine with hundreds of past sessions, switching it on used to bury the one session you were looking for under months of old ones - and it had to read every transcript ever written first. The scan now skips everything outside the window without opening it, which on a machine with 270 past sessions cuts the 24-hour listing from around two seconds to a fifth of one, and narrowing the window does not re-scan at all. The window also bounds what the content search reads, so the empty state now says so when nothing matched.
...
### Fixed
- A session you close after it has been sitting idle no longer vanishes from the overview on the spot. A finished session is meant to stay visible for an hour (`ended_max_age`), but that hour was counted from its last activity rather than from when it ended - so closing a window you had not typed in for a few hours dropped it in the very next refresh, and the only way back was the full list of older sessions. The hour now starts when the session ends.
- A project panel now keeps showing its most urgent session status ("Working", "Needs you", ...) in the header while the panel is open - it used to appear only once the panel was collapsed, so the group's headline vanished exactly when you opened the panel to look at it.
