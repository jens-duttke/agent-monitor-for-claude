...
- **Status derivation lives in the UI, not Python.** `snapshot.build_snapshot()` returns raw per-session records; `ui/logic.js` derives the status. Python never classifies. Status values (strings): `working`, `processing`, `interrupted`, `awaiting_input`, `awaiting_permission`, `new`, `completed`, `unknown`
