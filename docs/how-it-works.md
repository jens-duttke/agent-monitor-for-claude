...
The live overview above is built entirely from the session registry, so it only ever shows sessions that still have a registry record. The **Older** chip (off by default) adds the past sessions that no longer do: enabling it triggers a one-time, on-demand scan of the transcripts under `projects/`, skipping any session already shown live. Each past transcript is read once to resolve its title exactly as Claude Code shows it (a rename can sit anywhere in the file), its working directory (to group it under its project) and its last model and activity time. Token usage is deliberately not summed for history entries, which roughly halves that read. The scan runs on a worker thread, so the window never blocks, and the result is cached, so it is paid only the first time you enable the chip.

The chip's second half sets how far back it reaches, 24 hours by default. That window is not just a display filter: a transcript whose file has not been written inside it is skipped on its timestamp alone and never opened, so the default window reads only the last day's sessions instead of every session ever run. Narrowing the window is answered from the already-fetched result; only widening it scans again.

## WSL sessions

Everything above works the same way for a Claude Code session running natively inside a WSL distribution - a `claude` process whose own working directory and `.claude` configuration live inside the distribution's own Linux filesystem, not merely a Windows session whose Bash tool happens to invoke WSL for a command. Status, cost, subagents, history, search, and deletion all work identically; only where the files are read from differs. All of it depends on the `wsl` setting (see [configuration.md](configuration.md)), which is on by default. Turning it off also stops the periodic reads described below, which can otherwise keep a watched distribution from idling out.
...
Opening a path under `\\wsl.localhost\<distro>\` starts that distribution if it is not already running, so Agent Monitor only ever looks at one listed by `wsl.exe --list --running --quiet`. That command is the one program it ever runs, invoked by its absolute `System32` path, with fixed arguments, a hidden window, and a five-second timeout.

Two gates keep this near-free while WSL is not in use. A dedicated process-table pass - separate from the per-snapshot scan described above, and cached for five seconds - checks for a shared `vmmem*` process (the WSL2 utility VM); without it, no distribution can be running, so `wsl.exe` is never invoked at all. Once `vmmem` is seen, the running-distribution list itself is cached for 10 seconds, so a poll every few seconds does not re-run `wsl.exe` every time. When a check finds `vmmem` gone, both caches drop immediately, so a VM shutdown is noticed at most five seconds later instead of lingering for the ten-second discovery window.

One limitation is accepted rather than engineered away. If one distribution is stopped while another keeps the shared VM alive, a poll landing inside that 10-second cache window can still show the stopped one as running and read it once, which restarts it. It settles back down on its own: once it drops out of the `--running` list, nothing here keeps touching it, so it idles itself out again shortly after.

A WSL session's registry record carries the same `procStart` field a Windows one does, but it means something else there: it is exactly field 22 of `/proc/<pid>/stat` (`starttime`, ticks since boot), read directly over the same UNC mount - no subprocess involved. A pid missing from `/proc` is not alive. If a pid is present but its `stat` field 22 does not match the registry's `procStart`, Linux recycled it for an unrelated process, so it reads not alive too - the same recycled-pid guard the Windows probe applies, adapted to procfs's own field.
...
Some registry records additionally carry a `status` field (`busy`/`idle`, or `waiting` with a reason) maintained by Claude Code itself. When present it refines the derived status: `busy` reads as **Working**, `idle` as **Idle**, and a `waiting` record whose reason names a prompt as **Permission needed**. The last one is essential: Claude Code marks a session blocked on you *before* the pending tool request reaches the transcript, and worktree sessions share one transcript between two processes, which the transcript alone cannot tell apart. A detected permission prompt, an interrupt, or an API error is never overridden, and a `New` session is never demoted.
