...
The chip's second half sets how far back it reaches (one hour to 30 days, or everything; 24 hours by default). That window is not just a display filter: a transcript whose file has not been written inside it is skipped on its timestamp alone and never opened, so the default window reads only the last day's sessions instead of every session ever run. Narrowing the window is answered from the already-fetched result; only widening it scans again.
