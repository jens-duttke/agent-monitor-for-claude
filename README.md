...
- **Status at a glance** - each agent shows *working*, *waiting for you*, *permission needed*, *interrupted* (you stopped it mid-turn), or *finished*, with the time since its last activity.
- **Filter and sort to what matters** - the status chips (needs you, working, background, idle, interrupted, quiet, in that priority order) start all on, and you uncheck the ones you want to hide; each chip's colored dot doubles as the key to what the row dots mean; sort by activity, usage, model, host, or status. Your filter and sort choices are remembered across restarts. Freshly opened, never-used windows are hidden by default and one toggle away.
