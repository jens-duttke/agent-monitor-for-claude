...
- **Which Claude Code version wrote it** - a version column appears as soon as the agents in view are *not* all on the same Claude Code version, so you spot the long-running session still on last week's CLI. A session that outlived an update gets a "+N" badge listing every version it spanned and when each took over. Each version number opens that release's changelog entry in your browser.
