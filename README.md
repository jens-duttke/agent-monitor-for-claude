...
- **Filter and sort to what matters** - the status chips (needs you, working, background, error, idle, interrupted, quiet, new, in that priority order) start all on, and you uncheck the ones you want to hide; each chip's colored dot doubles as the key to what the row dots mean; sort by activity, usage, model, host, or status. Your filter and sort choices are remembered across restarts.
